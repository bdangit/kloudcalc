...
"click '#block-list,button.destroy'":"updateTotalCost"},initialize:function(){this.options.numBlocks&&(this.numBlocks=this.options.numBlocks);this.header=this.$("#header");this.totalCostField=this.$("#total-cost");this.blockList=this.$("#block-list");this.main=this.$("#main");this.footer=this.$("footer")},addComputeBlock:function(){var a=new e({model:new d,id:"compute-block-"+this.numBlocks,blockId:this.numBlocks});this.numBlocks+=1;this.blockList.append(a.render().el)},updateTotalCost:function(){var a=
0;this.$("#cost").each(function(){a+=parseFloat($(this).text().split("$")[1])});this.totalCostField.text("$"+a.toFixed(2))}}))});
